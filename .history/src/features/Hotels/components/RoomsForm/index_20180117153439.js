    ...
    <div className="RoomsForm">
      {fields.map((f, index) => (
        <div key={`${f.name}-${index}`} className="RoomsForm__RoomUpdate">
          <Header
            title={`${intl.formatMessage({ id: 'app.containers.SearchPage.room.room' })} #${index + 1}`}
          ...
          <div className="Form">
            <Field
              ...
              label={<Translation id="app.containers.SearchPage.room.adults" />}
            />
            <Field
              name={`${f}.child`}
              component={NumericInput}
              ...
              subLabel={<Translation id="app.containers.SearchPage.room.roomguestInfo" />}
              noBorder
            />
          </div>
        </div>
